...
W = 10
SHOW_S = 2.0
NOISE_LEVEL = "low"   # alpha=beta=0.05
...
SIG   = NAMES[SIGNAL_TO_EXTRACT].split()[0]
...
NC = config["noise"][NOISE_LEVEL]
ALPHA, BETA = NC["alpha"], NC["beta"]
...
def noisy_stitched(gen, n_wins, W, norm):
    """Fresh σ per window → noisy S5 mixture for display."""
...
# ── FIGURE 2: chosen signal — noisy S5 input vs clean target ─────────────────
fig, ax = plt.subplots(figsize=(14, 5))
fig.patch.set_facecolor(BG)
fig.suptitle(
    f"Source Separation — {NAMES[SIGNAL_TO_EXTRACT]}  "
    f"(α = β = {ALPHA}, noisy S5 input vs clean target)",
    fontsize=13, color="white")
style(ax)
noisy = noisy_stitched(gen, N_WINS, W, norms[4])
...
# ── TRAIN ─────────────────────────────────────────────────────────────────────
train_ds, val_ds = build_datasets(config, NOISE_LEVEL, window_size=W)
...
noisy_recon, preds = [], {name: [] for name in MCOL}
with torch.no_grad():
    for i in range(N_WINS):
        ...
        for name in MCOL:
            results[name]["model"].eval()
            preds[name].append(results[name]["model"](x).squeeze().numpy())
noisy_recon = np.concatenate(noisy_recon)
preds = {n: np.concatenate(p) for n, p in preds.items()}

# ── FIGURE 3: reconstruction — one subplot per model ─────────────────────────
fig, axes = plt.subplots(3, 1, figsize=(14, 11), sharex=True)
fig.patch.set_facecolor(BG)
fig.suptitle(
    f"Reconstruction — {NAMES[SIGNAL_TO_EXTRACT]}, α=β={ALPHA}, W={W}",
    fontsize=13, color="white")
...
ax.set_title(f"MSE Loss Curves — {NAMES[SIGNAL_TO_EXTRACT]}, α=β={ALPHA}, W={W}", fontsize=12)
...
ax.set_title(f"Best Val MSE — {NAMES[SIGNAL_TO_EXTRACT]}, α=β={ALPHA}, W={W}", fontsize=12)
...
print(f"\nDone. Change SIGNAL_TO_EXTRACT (0–3) at the top to run for other signals.")
