...
- Unique Skill: news/evidence analysis and logical fallacy detection on opponent claims.
- Must cite ≥1 live web source per argument.
- Must directly reference opponent's previous argument by round number.
- System prompt blocks agreement at the instruction level.
...
| 2 | Real contradiction via distinct Skills | Pro uses stats framing; Con uses evidence/fallacy skills; Father intervenes on overlap |
...
| 6 | No tie — winner declared | Father output contains winner field + scores (e.g., 58% vs 42%) |
...
## Assumptions
- Anthropic API key is available and funded for ~50+ API calls per run
...
- Web search depends on Tavily availability; system retries before failing gracefully
