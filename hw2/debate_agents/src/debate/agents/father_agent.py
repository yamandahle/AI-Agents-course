...
class FatherAgent(BaseAgent):
    ...
    def run_debate(self, topic: str, pro_agent: Any, con_agent: Any) -> DebateResult:
        ...
        for rnd in range(1, self._rounds + 1):
            ...
            pro_msg, n = self._check_and_intervene(pro_msg, pro_agent, current_msg, pro_history)
            interventions += n
            pro_history.append(pro_msg)
            transcript.append(pro_msg)

            con_msg = con_agent.generate_argument(pro_msg)
            con_msg, n = self._check_and_intervene(con_msg, con_agent, pro_msg, con_history)
            interventions += n
            ...
            if self._logger is not None:
                self._logger.info(
                    "father", "context_update",
                    {"round": rnd, "new_tokens": round_tokens, "total": context_tokens},
                )
                if rnd == self._summarize_after:
                    self._logger.info(
                        "father", "context_compact",
                        {"rounds_summarized": f"1-{rnd - 1}", "tokens_saved": context_tokens // 2},
                    )

            current_msg = con_msg

        return self._produce_verdict(pro_history, con_history, transcript, interventions, context_tokens)
    ...
    def _produce_verdict(
        ...
        return DebateResult(
            ...
            rounds_completed=len(pro_history),
